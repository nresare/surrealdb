...
pub(crate) fn default_headers() -> HeaderMap {
	...
	headers.insert(ACCEPT, HeaderValue::from_static(surrealdb_sdk_core::api::format::FLATBUFFERS));
	headers
		.insert(CONTENT_TYPE, HeaderValue::from_static(surrealdb_sdk_core::api::format::FLATBUFFERS));
	headers
}
...
async fn import(request: RequestBuilder, path: PathBuf) -> Result<()> {
	...
	let res = request
		.header(ACCEPT, surrealdb_sdk_core::api::format::FLATBUFFERS)
		.body(file)
		.send()
		.await
		.map_err(crate::std_error_to_types_error)?;
	...
	let value: Value = surrealdb_sdk_core::rpc::format::flatbuffers::decode(&bytes)
...
async fn send_request(
	...
	let body = surrealdb_sdk_core::rpc::format::flatbuffers::encode(&req_value)
	...
	let response: DbResponse = surrealdb_sdk_core::rpc::format::flatbuffers::decode(&bytes)
		.map_err(|x| format!("Failed to deserialize flatbuffers payload: {x}"))
		.map_err(|e| {
			crate::Error::internal(format!("The server returned an unexpected response: {e}"))
		})?;
...
async fn router(
	...
	match req.command {
		...
		Command::Set {
			...
			surrealdb_sdk_core::rpc::check_protected_param(&key)?;
			let req = Command::Set {
				key,
				value,
			}
