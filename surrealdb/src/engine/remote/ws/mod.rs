...
use crate::client_core::rpc::{DbResponse, DbResult};
...
fn serialize_request<M: WsMessage>(request: RouterRequest) -> M {
	...
	let payload = crate::client_core::rpc::format::flatbuffers::encode(&request_value)
...
async fn handle_parse_error(
	...
	match crate::client_core::rpc::format::flatbuffers::decode::<ErrorResponse>(binary) {
		Ok(ErrorResponse {
			id,
			session_id,
		}) => {
...
mod tests {
	...
	use crate::client_core::rpc::DbResult;
